...
CORES_POR_CUSTO = {
    "BilledCost": "#FF33A1",
    "EffectiveCost": "#00F2FE",
    "ContractedCost": "#9D4EDD",
    "ListCost": "#FFB142",
}
...
def _custos_disponiveis(df_focus):
    ...
    return [
        coluna
        for coluna in COLUNAS_DE_CUSTO
        if (
            coluna in df_focus.columns
            and df_focus[coluna].notna().any()
        )
    ]
...
def calcular_kpis(df_focus):
    ...
    colunas_valor = _custos_disponiveis(
        df_focus
    )


    total_faturado = (
        df_focus["BilledCost"].sum()
        if "BilledCost" in colunas_valor
        else None
    )


    total_efetivo = (
        df_focus["EffectiveCost"].sum()
        if "EffectiveCost" in colunas_valor
        else None
    )
    ...
    if (
        total_faturado is not None
        and total_efetivo is not None
        and total_faturado != 0
    ):
    ...
    if (
        "ServiceName" in df_focus.columns
        and "BilledCost" in colunas_valor
    ):
...
def grafico_por_dimensao(
    ...
    if coluna_dimensao == "ChargeCategory":
        ...
        fig = px.bar(
            ...
            y=col_v,
            text=col_v,
            title="Custos por ChargeCategory",
        )


        fig.update_traces(
            marker_color=CORES_POR_CUSTO.get(
                col_v,
                "#FF33A1"
            ),
...
def grafico_evolucao_temporal(
    ...
    # Linhas são mais adequadas que áreas empilhadas
    # porque os tipos de custo não devem ser somados
...
def gerar_graficos(df_focus):
    ...
    for coluna in COLUNAS_DIMENSAO:
        ...
        if (
            coluna
            in COLUNAS_IGNORADAS_SE_UNICAS
            and df_focus.get(
                coluna,
                pd.Series(
                    dtype=object
